...
public class SunTimeUtils {
    ...
    public static void main(String[] args) throws ParseException {
        System.out.println(getSunTime("Asia/Shanghai", 116.587099, 35.414921));
    }

    public static String[] getSunTime(String timeZone, Double longitude, Double latitude) throws ParseException {

        int days = ZonedDateTime.now(ZoneId.of(timeZone)).getDayOfYear();
        int timeZoneNo = TimeZone.getTimeZone(timeZone).getOffset(System.currentTimeMillis()) / 1000 / 60 / 60;

        Double riseDouble = getRise(timeZoneNo, days, longitude, latitude);
        Double setDouble = getSet(riseDouble, timeZoneNo, longitude);

        int riseHour = riseDouble.intValue();
        Double riseMin = (riseDouble - riseHour) * 60;
        int riseMinute = riseMin.intValue();
        String sunrise = (riseHour < 10 ? "0" + riseHour : riseHour) + ":" + (riseMinute < 10 ? "0" + riseMinute : riseMinute);

        int setHour = setDouble.intValue();
        Double setMin = (setDouble - setHour) * 60;
        int setMinute = setMin.intValue();
        String sunset = (setHour < 10 ? "0" + setHour : setHour) + ":" + (setMinute < 10 ? "0" + setMinute : setMinute);

        return new String[]{sunrise, sunset};
    }

    private static Double getRise(int timeZoneNo, int days, Double longitude, Double latitude) {
        return 24 * (180 + timeZoneNo * 15 - (longitude) - Math.acos(-Math.tan(-23.4 * Math.cos(2 * pi * (days + 9) / 365) * pi / 180) * Math.tan(latitude * pi / 180)) * 180 / pi) / 360;
    }

    private static Double getSet(Double rise, int timeZoneNo, Double longitude) {
        return 24 * (1 + (timeZoneNo * 15 - longitude) / 180) - rise;
    }
    ...
    private static void setStart(Double start) {
        SunTimeUtils.start = start;
    }

    private static Double getsRA() {
        return sRA;
    }

    private static void setsRA(Double sRA) {
        SunTimeUtils.sRA = sRA;
    }

    private static Double getSdec() {
        return sdec;
    }

    private static void setSdec(Double sdec) {
        SunTimeUtils.sdec = sdec;
    }

    private static Double getSr() {
        return sr;
    }

    private static void setSr(Double sr) {
        SunTimeUtils.sr = sr;
    }

    private static Double getLon() {
        return lon;
    }

    private static void setLon(Double lon) {
        SunTimeUtils.lon = lon;
    }
    ...
    private static void setEnd(Double end) {
        SunTimeUtils.end = end;
    }
